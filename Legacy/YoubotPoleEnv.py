...
from YoubotPoleSimModel import YoubotPoleSimModel
...
class YoubotPoleEnv(gym.Env):
    ...
    def __init__(self, action_type='discrete'):
        ...
        while True:
            client_ID = vrep_sim.simxStart('127.0.0.1', 19997, True, False, 5000, 1)
        ...
        self.youbot_pole_sim_model.initializeSimModel(client_ID)
        vrep_sim.simxSynchronousTrigger(client_ID)
    ...
    def step(self, action):
        ...
        q[0] = self.youbot_pole_sim_model.getJointPosition('prismatic_joint')[1]
        # Angle of the pole
        q[1] = self.youbot_pole_sim_model.getJointPosition('revolute_joint')
        self.q_last = self.q
        self.q = q
        ...
        if self.action_type == 'discrete':
            ...
            # The action is in [-1.0, 1.0], therefore the force is in [-20.0, 20.0]
            self.push_force = action*2.5
        else:
            assert 0, "The action type \'" + self.action_type + "\' can not be recognized"

        # set action
        self.youbot_pole_sim_model.setYoubotTorque(self.push_force)
        ...
        vrep_sim.simxSynchronousTrigger(self.youbot_pole_sim_model.client_ID)
        vrep_sim.simxGetPingTime(self.youbot_pole_sim_model.client_ID)
    ...
    def reset(self):
        ...
        vrep_sim.simxStopSimulation(self.youbot_pole_sim_model.client_ID, vrep_sim.simx_opmode_blocking) # stop the simulation
        vrep_sim.simxGetPingTime(self.youbot_pole_sim_model.client_ID)
        time.sleep(0.03) # ensure the coppeliasim is stopped
        ...
        vrep_sim.simxStartSimulation(self.youbot_pole_sim_model.client_ID, vrep_sim.simx_opmode_oneshot)
            
        self.youbot_pole_sim_model.setYoubotTorque(0)
        vrep_sim.simxSynchronousTrigger(self.youbot_pole_sim_model.client_ID)
        vrep_sim.simxGetPingTime(self.youbot_pole_sim_model.client_ID)
